module part3(SW, KEY, CLOCK_50, LEDR);
    ...
    LETTER_SELECTION L(SW, length, symbols);
...
module LENGTH_COUNTER(lengthIn, Clock, Resetn, enable, load, lengthOut); //enable: 0 = don't change, 1 = change to next symbol if not loading --- load: 0 = shift, 1 = load one symbol per FF and shift length # of times
    input [2:0] lengthIn;
...
module SHIFT_REGISTER(symbolsIn, Clock, Resetn, enable, load, symbolOut); //enable: 0 = don't change, 1 = change to next symbol if not loading --- load: 0 = shift, 1 = load one symbol per FF and shift length # of times
    input [3:0] symbolsIn;
    input Clock, Resetn, load, enable;
    output reg symbolOut; //catch the symbol that gets pushed out to use later (out of FF #4)
...
module Sync_half_second_cycle(Clock, Resetn, Q); //NOTE: CYCLES ARE MUCH LESS THAN 25 MILLION PER CHANGE FOR TESTING
    input Clock, Resetn;
    output reg Q;

    reg [24:0] value; //25 bits needed to rep. 25 mil
